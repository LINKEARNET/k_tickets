...
passport.use('local.signin', new LocalStrategy({
  ...
  const rows = await pool.query('SELECT * FROM terminales WHERE username_terminal = ?', [username_terminal]);
  if (rows.length > 0) {
    const user = rows[0];
	console.log(user);
...
passport.use(
  ...
  new LocalStrategy(
    ...
    async (req, username_terminal, password_terminal, done) => {
      ...
      const result = await pool.query('INSERT INTO terminales SET ? ', newUser);
